...
type AuthService interface {
	...
	LoginWithOAuthProvider(ctx context.Context, email string, providerID string, oauthProvider string) (*domain.Tokens, error)
...
func (s *authService) LoginWithOAuthProvider(ctx context.Context, email string, providerID string, oauthProvider string) (*domain.Tokens, error) {
	if email == "" {
		return nil, domain.ErrEmailNotProvided
	}

	// Try to find user by provider id and return tokens if user was found.
	u, err := s.authRepo.FindUserByProviderID(ctx, providerID, oauthProvider)
	if err == nil {
		accessToken, err := s.tokenService.GenerateAccessToken(u.UserID)
		if err != nil {
			return nil, fmt.Errorf("generate access token: %w", err)
		}
		...
		h, expiresAt := s.tokenService.HashRefreshToken(refreshToken)
		if err := s.tokenRepo.SaveHashedRefreshToken(ctx, u.UserID, h, expiresAt); err != nil {
			if errors.Is(err, repository.ErrGatewayTimeout) {
				return nil, domain.ErrGatewayTimeout
			} else {
				return nil, fmt.Errorf("save hashed refresh token: %w", err)
		...
		up, err := s.authRepo.FindUserByEmail(ctx, email)
		if err != nil {
			if errors.Is(err, repository.ErrNotFound) {
				u, err := s.authRepo.CreateOAuthUser(ctx, email, providerID, oauthProvider)
				if err != nil {
					if errors.Is(err, repository.ErrGatewayTimeout) {
						return nil, domain.ErrGatewayTimeout
					} else {
			...
			accessToken, err := s.tokenService.GenerateAccessToken(up.UserID)
			if err != nil {
				return nil, fmt.Errorf("generate access token: %w", err)
			}
			refreshToken, err := s.tokenService.GenerateRefreshToken()
			...
			h, expiresAt := s.tokenService.HashRefreshToken(refreshToken)
			if err := s.tokenRepo.SaveHashedRefreshToken(ctx, up.UserID, h, expiresAt); err != nil {
				if errors.Is(err, repository.ErrGatewayTimeout) {
					return nil, domain.ErrGatewayTimeout
				} else {
					return nil, fmt.Errorf("save hashed refresh token: %w", err)
...
func (s *authService) UserInfo(ctx context.Context, userID uuid.UUID) (*domain.User, error) {
	u, err := s.authRepo.GetUserInfo(ctx, userID)
	if err != nil {
		if errors.Is(err, repository.ErrNotFound) {
